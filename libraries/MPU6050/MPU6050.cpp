...
void MPU6050::getMotion6(float *ax, float *ay, float *az, float *gx, float *gy, float *gz)
{
    ...
    uint8_t gxha = Wire.read();
	uint8_t gxla = Wire.read();
    uint8_t gyha = Wire.read();
	uint8_t gyla = Wire.read();
	uint8_t gzha = Wire.read();
	uint8_t gzla = Wire.read();
	
    *ax = (xha << 8 | xla) * rangePerDigit * 9.80665f;
    *ay = (yha << 8 | yla) * rangePerDigit * 9.80665f;
    *az = (zha << 8 | zla) * rangePerDigit * 9.80665f;

    if (useCalibrate)
    {
	    *gx = ((gxha << 8 | gxla) - dg.XAxis) * dpsPerDigit;
	    *gy = ((gyha << 8 | gyla) - dg.YAxis) * dpsPerDigit;
	    *gz = ((gzha << 8 | gzla) - dg.ZAxis) * dpsPerDigit;
    } 
    else
    {
	    *gx = (gxha << 8 | gxla) * dpsPerDigit;
	    *gy = (gyha << 8 | gyla) * dpsPerDigit;
	    *gz = (gzha << 8 | gzla) * dpsPerDigit;
    }

    if (actualThreshold)
    {
	    if (abs(*gx) < tg.XAxis) *gx = 0;
    	if (abs(*gy) < tg.YAxis) *gy = 0;
	    if (abs(*gz) < tg.ZAxis) *gz = 0;
    }
